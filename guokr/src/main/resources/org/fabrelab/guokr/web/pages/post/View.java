...
import org.fabrelab.guokr.web.pages.group.Posts;
...
public class View {
	...
	Link onActionFromDelete(EventContext context) throws NoPermissionException {
		...
		List<GroupDO> groups = getPostGroups();
		for (GroupDO group : groups) {
			Link link = pageRenderLinkSource.createPageRenderLinkWithContext(Posts.class, group.getId());
			return link;
		}
		Link link = pageRenderLinkSource.createPageRenderLinkWithContext(org.fabrelab.guokr.web.pages.user.View.class, post.getCreatorId());
		return link;
	}
}
